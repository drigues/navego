...
<div x-data="{
    ...
    editModal: false,
    editData: {},
    openEdit(cat) { this.editData = cat; this.editModal = true; }
}">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-5 py-3">Slug</th>
                        <th class="text-center text-xs font-semibold text-gray-500 uppercase tracking-wide px-5 py-3">Prestadores</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($categories as $cat)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-3.5 text-xs text-gray-400 font-mono">{{ $cat->slug }}</td>
                            <td class="px-5 py-3.5 text-center text-gray-700">{{ $cat->providers_count }}</td>
                            <td class="px-5 py-3.5">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-2 justify-end">
                                    <button @click="openEdit({{ json_encode(['id' => $cat->id, 'name' => $cat->name, 'description' => $cat->description, 'icon' => $cat->icon, 'is_active' => $cat->is_active]) }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center text-sm text-gray-400">Nenhuma categoria encontrada.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form :action="`/admin/categorias/${editData.id}`" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2" x-text="editData.description"
                ...
                <div>
                    ...
                    <input type="text" name="icon" :value="editData.icon"
                           class="w-full rounded-lg border-gray-300 shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="⚖️">
                </div>
