    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-4 divide-x divide-gray-100">
            @foreach([
                ...
                ['value' => $stats['services'],  'label' => 'Orçamentos'],
            ...
            @foreach($categories as $category)
                <a href="{{ route('servicos.index', ['categoria' => $category->slug]) }}"
                   class="group bg-white rounded-2xl p-5 shadow-sm hover:shadow-md border border-gray-100 hover:border-indigo-100 transition-all duration-200 flex flex-col items-start gap-3">
                    <div class="w-11 h-11 rounded-xl bg-indigo-600 flex items-center justify-center text-white font-bold text-lg shrink-0 transition-transform group-hover:scale-110">
                    ...
                    <div>
                        ...
                        @if(($category->providers_count ?? 0) > 0)
                            <p class="text-xs text-gray-400 mt-0.5">{{ $category->providers_count }} {{ $category->providers_count === 1 ? 'prestador' : 'prestadores' }}</p>
                        @endif
                    </div>
                </a>
            @endforeach
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-end justify-between mb-12">
            <div>
                ...
                <p class="text-gray-500">Profissionais com mais pedidos de orçamento.</p>
            </div>
            ...
            @foreach($featuredProviders as $provider)
                <a href="{{ route('servicos.show', $provider->slug) }}"
                   class="group bg-white rounded-2xl border border-gray-100 hover:border-indigo-100 shadow-sm hover:shadow-md p-6 transition-all duration-200 flex flex-col">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-gray-50 mt-auto text-xs text-gray-400">
                        @if($provider->category)
                            <span class="bg-gray-100 text-gray-600 px-2.5 py-1 rounded-lg font-medium">{{ $provider->category->name }}</span>
                        @endif
                        @if($provider->quotes_count > 0)
                            <span>{{ $provider->quotes_count }} {{ $provider->quotes_count === 1 ? 'pedido' : 'pedidos' }}</span>
                        @endif
                    </div>
                </a>
            @endforeach
            ...
            @foreach($recentGuides as $guide)
                <a href="{{ route('guias.show', $guide->slug) }}"
                   class="group bg-indigo-900/50 hover:bg-indigo-800/60 border border-indigo-800 hover:border-indigo-600 rounded-2xl p-6 transition-all duration-200">
                    @if($guide->category)
                        <span class="inline-block text-xs font-medium text-indigo-300 bg-indigo-800 px-2.5 py-1 rounded-lg mb-3">{{ $guide->category }}</span>
                    @endif
